...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        if (mpInitializer->Initialize(mCurrentFrame, mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated))
        {
            ...
            //cout << Converter::toVector3d(mCurrentFrame.GetCameraCenter()) << endl;
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    pKFcur->SetPose(Tc2w);
    //cout << Converter::toVector3d(pKFcur->GetCameraCenter()) << endl;
...
bool Tracking::TrackLocalMap()
{
    ...
    // We retrieve(检索) the local map and try to find matches to points in the local map.
...
void Tracking::CreateNewKeyFrame()
{
    ...
    // 双目情况下增加MapPoint
    // 单目情况下，tracking线程只跟踪已有的MapPoint，mapping线程用来增加MapPoint
    if (mSensor != System::MONOCULAR)
    {
        ...
        if (!vDepthIdx.empty())
        {
            ...
            for (size_t j = 0; j < vDepthIdx.size(); j++)
            {
                ...
                bool bCreateNew = false;
                // 创建新的MapPoint；
                // mvpMapPoints的大小为orb提取器提取到的orb特征点的数量，在frame构造函数中已经指定；
                MapPoint *pMP = mCurrentFrame.mvpMapPoints[i];
                if (!pMP)
                    bCreateNew = true;
                else if (pMP->Observations() < 1)
